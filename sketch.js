...
function draw() { 
  ...
  if(!gameOver){
    ...
    if(greenLightningGroup.isTouching(electrofast)||dronesGroup.isTouching(electrofast)){
      winner = "gadgetack"
      gameOver=true
    }

    if(lightningGroup.isTouching(gadgetack)||dronesGroup.isTouching(gadgetack)){
      winner = "electrofast"
      gameOver=true
    }

    if(dronesGroup.isTouching(gadgetack)){
...
function buildDrones(){
  ...
  if(dronesGroup.length>0) {
    for(var d=0; d<dronesGroup.length;d++) {
      if(dronesGroup[d].isTouching(lightningGroup)||dronesGroup[d].isTouching(greenLightningGroup)){
        dronesGroup[d].destroy()
      }
    }  
  }  
}
